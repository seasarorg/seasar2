...
public class DefaultConventionNaming implements ConventionNaming {
    ...
    public String defineName(String rootPackageName, String[] middlePackageNames,
            String nameSuffix, String className) {
    ...
    protected String trimMiddlePackageNames(String name, String[] packageNames) {
        if (packageNames != null && packageNames.length > 0) {
            ...
            throw new IllegalArgumentException(name + ", " + Arrays.asList(packageNames));
        }
        return name;
    }
    ...
    protected String composeName(String nameSuffix, String name) {
        ...
        boolean hasImplementatinPackage = lastName.length() > pos + nameSuffix.length();
        lastName = StringUtil.decapitalize(lastName.substring(0, pos)) + nameSuffix;
        if (names.length == 3) {
