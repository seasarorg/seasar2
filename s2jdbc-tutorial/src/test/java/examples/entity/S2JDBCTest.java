...
public class S2JDBCTest extends S2TestCase {
	...
	private static final String SQL_FILE = "examples/sql/employee/selectWithDept.sql";

	private static final String SQL_FILE2 = "examples/sql/employee/selectWithDept2.sql";
	...
	public void testManyToOne() throws Exception {
		List<Employee> results = jdbcManager.from(Employee.class).join(
	...
	public void testOneToMany() throws Exception {
		List<Department> results = jdbcManager.from(Department.class).join(
			"employeeList").getResultList();
	...
	public void testOneToOneNonowner() throws Exception {
		List<Address> results = jdbcManager
			.from(Address.class)
			.join("employee")
			.getResultList();
	...
	public void testOneToManyWhere() throws Exception {
		List<Department> results = jdbcManager.from(Department.class).join(
			"employeeList").where("employeeList.id = ?", 1).getResultList();
		for (Department d : results) {
			System.out.println(d.name);
			for (Employee e : d.employeeList) {
				System.out.println(" " + e.name);
	...
	public void testOneToOneOwnerWhere() throws Exception {
		List<Employee> results = jdbcManager
			...
			.join("address")
			.where(
				new SimpleWhere().starts("name", "A").ends("address.name", "1"))
			.getResultList();
		for (Employee e : results) {
			System.out.println(e.name + ", " + e.address.name);
		}
	...
	public void testOrderBy() throws Exception {
		List<Employee> results = jdbcManager.from(Employee.class).orderBy(
			"name desc").getResultList();
		for (Employee e : results) {
			System.out.println(e.name);
		}
	}
